...
int dbfd=-1;                       /* File descriptor for reading the DB */
int mxPage=0;                     /* Last page number */
int perLine=16;                    /* HEX elements to print per line */
int bRaw=0;                       /* True to access db file via OS APIs */
sqlite3 *pDb=NULL;


/*
** Convert the var-int format into int.  Return the number of bytes
...
static void fileOpen(const char *zPrg, const char *zName){
  ...
  if( bRaw==0 ){
    int rc;
    void *pArg=NULL;
    pDb = openDatabase(zPrg, zName);
    rc = sqlite3_file_control(pDb, "main", SQLITE_FCNTL_FILE_POINTER, pArg);
    if( rc!=SQLITE_OK ){
      fprintf(stderr, 
...
*/
static unsigned char *fileRead(int ofst, int nByte){
  unsigned char *aData;
  int got;
  ...
  if( bRaw==0 ){
    ...
    _lseek(dbfd, (long)ofst, SEEK_SET);
    got = _read(dbfd, aData, nByte);
    ...
    for(j=0; j<perLine; j++){
      if( i+j>nByte ){
        fprintf(stdout, " ");
		  ...
		  fprintf(stdout, "%c", isprint((unsigned char)(aData[i + j])) ? aData[i + j] : '.');
      }
    }
    fprintf(stdout,"\n");
  }
...
** the page size.
*/
static int localPayload(int nPayload, char cType){
  int maxLocal;
  int minLocal;
  int surplus;
  int nLocal;
       ...
       if( typeCol[i]<=7 ){
         ...
         if( (typeCol[i]&1)==0 ){
           ...
           for(ii=1, jj=0; jj<szCol[i] && ii<24; jj++, ii++){
			   zConst[ii] = isprint((unsigned char)(pData[jj])) ? pData[jj] : '.';
           }
           zConst[ii] = 0;
         }
         if( jj<szCol[i] ){
...
static void decode_trunk_page(
  ...
  unsigned char *a;
  while( pgno>0 ){
    a = fileRead((pgno-1)*pagesize, pagesize);
...
static void ptrmap_coverage_report(const char *zDbName){
  ...
  for(pgno=2; pgno<=mxPage; pgno += perPage+1){
    printf("%5d: PTRMAP page covering %d..%d\n", pgno,
           pgno+1, pgno+perPage);
    a = fileRead((pgno-1)*pagesize, usable);
...
int main(){
	int argc = 3;
	char* argv[] = { "showdb.exe", "-raw", "sdu.db" };
  int szFile;
  unsigned char *zPgSz;
  ...
  if( nArg>1 ){
	  if (sqlite3_stricmp("-raw", "-raw") == 0
		  || sqlite3_stricmp("--raw", "-raw") == 0
    ){
      bRaw = 1;
      azArg++;
      nArg--;
  ...
  printf("Pagesize: %d\n", pagesize);
  mxPage = (int)((szFile+pagesize-1)/pagesize);
