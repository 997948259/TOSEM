...
sqlite3 *db;
sqlite3_stmt *pStmt;
static void prepareAndRun(const char *zSql){
...
int main(){
	int argc = 8;
	char* argv[] = {"speedtest8inst1.exe", "-db", "speedtest8.db", "-script", "speedtest8.sql", "-log", "log.txt", "-logdata"};
  ...
  _unlink(zDb);
  rc = sqlite3_open_v2(
     zDb, &db, SQLITE_OPEN_READWRITE | SQLITE_OPEN_CREATE, NULL
  );
  if( rc!=SQLITE_OK ){
