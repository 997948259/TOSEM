...
struct Str {
  ...
  int n;       /* Bytes of input used */
  int nAlloc;  /* Bytes allocated to z[] */
...
static void runSql(sqlite3 *db, const char *zSql, unsigned  runFlags){
  ...
  while( zSql && zSql[0] ){
    zMore = 0;
    stmt = 0;
    sqlite3_prepare_v2(db, zSql, -1, &stmt, &zMore);
    assert( zMore<=zEnd );
    if( zMore==zSql ) break;
    if( runFlags & 1 ){
      const char *z = zSql;
    ...
    if( stmt ){
      if( (runFlags & 2)==0 ){
        ...
        int nCol = -1;
        int nRow;
        for(nRow=0; SQLITE_ROW==sqlite3_step(stmt); nRow++){
          int i;
          if( nCol<0 ){
            nCol = sqlite3_column_count(stmt);
          }
          for(i=0; i<nCol; i++){
            int eType = sqlite3_column_type(stmt,i);
            printf("ROW[%d].%s = ", nRow, sqlite3_column_name(stmt,i));
            switch( eType ){
              case SQLITE_NULL: {
                printf("NULL\n");
                break;
