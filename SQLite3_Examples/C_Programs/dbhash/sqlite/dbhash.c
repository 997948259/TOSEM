...
static void hash_step(const unsigned char *data, unsigned int len){
	...
	j = (j >> 3) & 63;
	if ((j + len) > 63){
		memcpy(&g.cx.buffer[j], data, (i = 64 - j));
	...
	memcpy(&g.cx.buffer[j], &data[i], len - i);
}
...
*/
sqlite3_stmt *pStmt1;
static void my_hash_one_query(){

	int nCol;                   /* Number of columns in the result set */
	int i;                      /* Loop counter */

	nCol = sqlite3_column_count(pStmt1);

	/* Compute a hash over the result of the query */
	while (SQLITE_ROW == sqlite3_step(pStmt1)){
		for (i = 0; i<nCol; i++){
			switch (sqlite3_column_type(pStmt1, i)){
			case SQLITE_NULL: {
				hash_step((const unsigned char*)"0", 1);
			...
			case SQLITE_INTEGER: {
				...
				int j;
				unsigned char x[8];
				sqlite3_int64 v = sqlite3_column_int64(pStmt1, i);
				memcpy(&u, &v, 8);
				for (j = 7; j >= 0; j--){
					x[j] = u & 0xff;
					u >>= 8;
			...
			case SQLITE_FLOAT: {
				...
				int j;
				unsigned char x[8];
				double r = sqlite3_column_double(pStmt1, i);
				memcpy(&u, &r, 8);
				for (j = 7; j >= 0; j--){
					x[j] = u & 0xff;
					u >>= 8;
			...
			case SQLITE_TEXT: {
				int n = sqlite3_column_bytes(pStmt1, i);
				const unsigned char *z = sqlite3_column_text(pStmt1, i);
			...
			case SQLITE_BLOB: {
				int n = sqlite3_column_bytes(pStmt1, i);
				const unsigned char *z = sqlite3_column_blob(pStmt1, i);
...
int main(){
	...
	static const int openFlags =
		...
		;
	zDb = "dbhash.db";
	rc = sqlite3_open_v2(zDb, &db, openFlags, 0);
	if (rc){
	...
	if (!omitContent){
		...
		while (SQLITE_ROW == sqlite3_step(pStmt)){
			...
			** can be safely omitted. */
			hash_one_query("SELECT * FROM \"%s\"", sqlite3_column_text(pStmt, 0));
		}
